<template>
	...
	<div class="inline-flex">
		<span
			...
			class="text-success items-center"
		>
			<Icon
				:icon="'Complete'"
		...
		<span
			...
			class="text-attention items-center"
		>
			<Icon
				:icon="'InProgress'"
				:class="'inline-block h-auto w-6 align-middle'"
	...
	<table class="pkpTable w-full">
		...
		<template v-for="(item, i) in itemsFiltered" :key="i">
			<tr>
				...
				<td>
					<input v-model="item.creators" type="text" class="input w-full" />
				</td>
				<td>
					<input v-model="item.publisher" type="text" class="input w-full" />
				</td>
				<td class="w-5rem">
					<input
...
const userCanEdit = computed(() =>
	editorRoles.some((item) => pkp.currentUser.roles.includes(item)),
);
